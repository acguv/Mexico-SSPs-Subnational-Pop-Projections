...
derive_int_out_migration <- function(inputs_path, cur_year, pop_df, cur_scenario, int_mig_factor){
  ...
  for (region in regions){
    ...
    cur_int_mig_rates_df <- file.path(inputs_path, region, 'internal_out_migration_ent.csv') %>%
      fread() %>%
    ...
    cur_tot_int_mig_values_df <- cur_int_mig_values_df[, .(int_out_mig = sum(int_out_mig)*int_mig_factor),
                                                       by = .(Region, Race, Sex, Age)]
